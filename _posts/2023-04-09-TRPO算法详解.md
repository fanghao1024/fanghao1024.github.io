...
最终TRPO的目标转化为转化为：
$$
\max_s E_{s \sim \rho_{\theta_{old}},a \sim \pi_{\theta_{old}}}[\frac{\pi_{\theta}(a|s)}{\pi_{\theta_{old}}(a|s)}A_{\theta_{old}}(s,a)]\\
...
> $$
> \begin{aligned}
> \nabla_\theta D_{KL}(\pi_{\theta}(\cdot|s)||\pi_{\theta_{old}}(\cdot|s))&=-\int \pi_{\theta_{old}}(x|s))\nabla_{\theta}\log\pi_{\theta}(x|s))\,dx\\
...
> $$
> \begin{aligned}
> \nabla_\theta^2 D_{KL}(\pi_{\theta}(\cdot|s)||\pi_{\theta_{old}}(\cdot|s))&=-\int \pi_{\theta_{old}}(x|s))\nabla_{\theta}^2\log\pi_{\theta}(x|s))\,dx \longrightarrow 记H=\nabla_{\theta}^2\log\pi_{\theta}(x|s))\\
> &=-\int \pi_{\theta_{old}}(x|s)H_{\log \pi_{\theta}}\,dx\\
> &=-E_{\theta_{old}}[H_{\log \pi_{\theta}}]\\
...
利用KKT条件：
$$
\frac{\partial L(\theta,\lambda)}{\partial \theta}=-\nabla_\theta L_{\theta_{old}}(\theta)|_{\theta=\theta_{old}}+\lambda F(\theta_{old})(\theta-\theta_{old})=0 \tag{24}
$$
...
>
> ​	a)	 $\alpha_k=-\frac{(g^{(k)})^Td^{k}}{(d^k)^TQd^{k}}$
>
> ​	b)	$\lambda^{(k+1)}=\lambda^{(k)}+\alpha_kd^{(k)}$
>
> ​	c)	$g^{(k+1)}=\nabla f(x^{(k+1)})=Ax^{(k+1)}-b$，如果$g^{(k+1)}=0$，停止迭代
>
> ​	d)	$\beta_k=\frac{(g^{(k)})^Td^{k}}{(d^k)^TQd^{k}}$
>
